...
public class SavesManager : MonoBehaviour
{
    ...
    public void GetSaveData(int Hour, string Month)
    {
        for (int i = 0; i < SavesButton.Count; i++)
        {
            SavesButton[i].TotalHoursText.text = $"{Hour}";
            SavesButton[i].MonthName.text = Month ;
        }
    }
